...
void IoT_sink::recv_handler (packet *p){
    ...
    if (p->type() == "IoT_ctrl_packet" && !hi ) { // the device receives a packet from the sink
        ...
        //map<pair<unsigned int, unsigned int>, int> completeGraph = convertToCompleteGraph(graph);
        

        /*
        Graph lasttree(getNodeNum());
        
        /*
        // Accessing elements put them to last struct
        for (const auto& entry : completeGraph) {
            const pair<int, int>& nodes = entry.first;
            int weight = entry.second;
            //cout << "From node " << nodes.first << " to node " << nodes.second << ", weight: " << weight << endl;
            lasttree.addEdge(nodes.first,nodes.second,weight);
        }
        */
        ...
        /*
        //test
        lasttree.addEdge(0,1,2);
        lasttree.addEdge(0,2,3);
        lasttree.addEdge(0,3,3);
        lasttree.addEdge(0,4,2);
        lasttree.addEdge(1,0,2);
        lasttree.addEdge(1,2,2);
        lasttree.addEdge(1,3,2);
        lasttree.addEdge(1,4,2);
        lasttree.addEdge(2,0,2);
        lasttree.addEdge(2,1,2);
        lasttree.addEdge(2,3,2);
        lasttree.addEdge(2,4,2);
        lasttree.addEdge(3,0,2);
        lasttree.addEdge(3,1,2);
        lasttree.addEdge(3,2,2);
        lasttree.addEdge(3,4,2);
        lasttree.addEdge(4,0,2);
        lasttree.addEdge(4,1,2);
        lasttree.addEdge(4,2,2);
        lasttree.addEdge(4,3,2);
        

        //set root point at 0 and excute
        lasttree.LAST(0);
        */
        ...
        */
        ...
        //map<unsigned int, unsigned int> spt = dijkstra(0, completeGraph, getNodeNum());
